# Launch and Indexing Checklist

Local pre-launch verification completed 2026-05-22:

- [x] Local robots.txt loads
- [x] Local sitemap.xml loads
- [x] Local robots.txt references sitemap.xml
- [x] Local sitemap.xml includes homepage
- [x] Local sitemap.xml includes /tools
- [x] Local sitemap.xml includes /guides
- [x] Local sitemap.xml includes /methodology
- [x] Local sitemap.xml includes /home-battery-quote-comparison-uk

Live domain checks below are intentionally left open until `https://homebatterysavings.co.uk` is reachable and dashboard access is available.

## Live crawl files

- [ ] https://homebatterysavings.co.uk/robots.txt loads
- [ ] https://homebatterysavings.co.uk/sitemap.xml loads
- [ ] robots.txt references sitemap.xml
- [ ] sitemap.xml includes homepage
- [ ] sitemap.xml includes /tools
- [ ] sitemap.xml includes /guides
- [ ] sitemap.xml includes /methodology
- [ ] sitemap.xml includes /home-battery-quote-comparison-uk

## Google Search Console

- [ ] Domain property added
- [ ] Ownership verified
- [ ] sitemap.xml submitted
- [ ] Homepage inspected
- [ ] /tools inspected
- [ ] /guides inspected
- [ ] /methodology inspected
- [ ] /home-battery-quote-comparison-uk inspected
- [ ] /home-battery-savings-examples-uk inspected

## Bing Webmaster Tools

- [ ] Site added or imported from Google Search Console
- [ ] Ownership verified
- [ ] sitemap.xml submitted
- [ ] Homepage checked
- [ ] /tools checked
- [ ] /guides checked
- [ ] /home-battery-quote-comparison-uk checked

## First-week follow-up

- [ ] Check Google sitemap status
- [ ] Check Bing sitemap status
- [ ] Check indexed pages
- [ ] Check crawl errors
- [ ] Check Vercel Analytics
- [ ] Check Vercel Speed Insights
- [ ] Fix any 404s or duplicate URL issues
